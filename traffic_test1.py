...
MODEL_PATH = "XGBoost_Traffic_Model.pkl"
...
def load_model():
    ...
    return model, label_encodings


def fetch_real_time_data(lat, lon, timezone, label_encodings):
    ...
    try:
        ...
        return {
            "Latitude": lat,
            "Longitude": lon,
            "Day_Sin": round(math.sin(2 * math.pi * local_time.weekday() / 7), 6),
            ...
            "hour_sin": round(math.sin(2 * math.pi * local_time.hour / 24), 6),
...
def test_model():
    ...
    model, label_encodings = load_model()
    lat, lon, timezone = 28.6139, 77.2090, "Asia/Kolkata"  # Example: Delhi, India
    
    data = fetch_real_time_data(lat, lon, timezone, label_encodings)
    if data:
